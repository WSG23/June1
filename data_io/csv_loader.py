...
def load_csv_event_log(csv_file_obj, column_mapping, timestamp_format=None):
    ...
    try:
        ...
        if timestamp_format:
            ...
            # Try common timestamp formats first to avoid parsing warnings
            common_formats = [
                '%Y-%m-%d %H:%M:%S',
                '%Y-%m-%d %H:%M:%S.%f',
                '%Y-%m-%dT%H:%M:%S',
            ...
            for fmt in common_formats:
                try:
                    event_df[TIMESTAMP_COL_DISPLAY] = pd.to_datetime(event_df[TIMESTAMP_COL_DISPLAY], format=fmt, errors='raise')
            ...
            if not parsed_successfully:
                print("Warning: Using default timestamp parsing - this may be slow for large datasets")
